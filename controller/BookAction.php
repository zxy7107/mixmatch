...
require_once '../phpbean/Book.class.php';  //导入实体类
require_once '../service/BookService.class.php';  //导入业务层类
...
if(!empty($_REQUEST['action'])) {
	...
	if($action == "add") {
		...
		$bookService->getFenyePage($fenyePage,$barcode,$bookName,$bookType,$publishDate);  
		...
		//说明用户希望执行更新图书信息   
		$array_book = $_POST['book']; 
		$book = $bookService->GetBook($array_book['barcode']);
		...
		if($bookService->DeleteBook($barcode) == 1) {
			//成功
			header("Location: ../ok.php");
			exit();
		} else {
